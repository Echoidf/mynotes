...
| **网络通信层组件**  | **说明**                                     |
| ------------------- | -------------------------------------------- |
| **BootStrap**       | **负责客户端启动并用来连接远程Netty Server** |
| **ServerBootStrap** | **负责服务端监听，用来监听指定端口**         |
| **Channel**         | **完成网络通信的载体**                       |

| **事件调度层组件   ** | **说明**                                                     |
| --------------------- | ------------------------------------------------------------ |
| **EventLoopGroup**    | **本质上是一个线程池，主要负责接收IO请求，并分配线程执行处理请求** |
| **EventLoop**         | **相当于线程池中的线程**                                     |

| **服务编排层组件**       | **说明**                                                    |
| ------------------------ | ----------------------------------------------------------- |
| **ChannelPipeline**      | **负责将多个ChannelHandler链接在一起**                      |
| **ChannelHandle**        | **针对IO的数据处理器数据接收后，通过指定的Handler进行处理** |
| **ChannelHanlerContext** | **用来保存ChannelHandler的上下文信息**                      |
